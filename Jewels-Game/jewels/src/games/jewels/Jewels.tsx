...
class Jewels extends React.Component<{}, AppState> {
  ...
  reset = () => {    
    ...
    const randomArray = this.getRandomNumbers(dCount, maxPlots);
  ...
  randomNumber  = (max:number, min:number, randomArray:Array<number>): number => {
    const num = Math.floor(Math.random() * (max - min +1)) + min;
    return randomArray.indexOf(num) >= 0 ? this.randomNumber(max, min, randomArray) : num;
 }

 // Get random numbers for diamond plotting
  getRandomNumbers = (dcount:number, max:number) => {
    const min = 1;    
    const randomArray:Array<number> =  []  ;
    for(let i = min; i <= dcount; i++) {      
      randomArray[i-1] = this.randomNumber(0, max, randomArray)       
    }       
    return randomArray;
  }
